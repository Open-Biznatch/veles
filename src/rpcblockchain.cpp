...
double GetPoWMHashPS()
{
    ...
    int nPoWInterval = 72;
    int64_t nTargetSpacingWorkMin = 120, nTargetSpacingWork = 120;

    CBlockIndex* pindex = pindexBestHeader;
    CBlockIndex* pindexPrevWork = pindexBestHeader;
...
Object blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zvelesObj.push_back(Pair(to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zvelesObj.emplace_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.emplace_back(Pair("zVLSsupply", zvelesObj));
