...
static const int IDLE_TIMEOUT = 9 * 60;
/** Time between cycles to check for idle nodes, force disconnect (seconds) **/ 
static const int DATA_TIMEOUT = 10 * 60;
...
public:
    ...
    CService addrLocal;
    int nVersion;
    // strSubVer is whatever byte array we read from the veles. However, this field is intended
    // to be printed out, displayed to humans in various forms and so on. So we sanitize it and
    // store the sanitized version in cleanSubVer. The original should be used when dealing with
    // the network or veles types and the cleaned string used when displayed or logged.
