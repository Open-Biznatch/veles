...
#include "zvelescontroldialog.h"
#include "ui_zvelescontroldialog.h"
...
std::list<std::string> ZVelesControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZVelesControlDialog::listMints;

ZVelesControlDialog::ZVelesControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZVelesControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZVelesControlDialog::~ZVelesControlDialog()
{
    delete ui;
}

void ZVelesControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZVelesControlDialog::updateList()
{
...
void ZVelesControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZVelesControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZVeles_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZVelesControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZVelesControlDialog::GetSelectedMints()
{
...
void ZVelesControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
