...
void PrivacyDialog::on_pushButtonZVelesControl_clicked()
{
    ZVelesControlDialog* zVelesControl = new ZVelesControlDialog(this);
    zVelesControl->setModel(walletModel);
    zVelesControl->exec();
}

void PrivacyDialog::setZVelesControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzVelesSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzVLS()
{
    ...
    // use mints from zVeles selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZVelesControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZVelesControlDialog::GetSelectedMints();
    }
    ...
    // Clear zveles selector in case it was used
    ZVelesControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zVeles Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Veles, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zVeles Mint");
